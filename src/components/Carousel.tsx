...
export interface CarouselItem {
  ...
  testimonials?: readonly CarouselTestimonial[];
}
        ...
        <div className="grid h-full grid-cols-1 gap-4 md:grid-cols-3 md:gap-6">
          {item.testimonials.map((testimonial) => (
            <article
              ...
              className="flex h-full flex-col rounded-2xl border border-white/10 bg-black/80 p-6 backdrop-blur-md"
            >
              <span className="mb-4 flex size-8 items-center justify-center rounded-full border border-emerald-300/30 bg-emerald-300/10">
                {item.icon}
              </span>
              <p className="flex-1 text-base leading-8 text-white/85 md:text-lg md:leading-9">
                „{testimonial.quote}”
              </p>
              <p className="mt-5 text-sm font-medium text-emerald-300 md:text-base">{testimonial.author}</p>
            </article>
          ))}
        </div>
      </motion.div>
  ...
  return (
    <div
      ...
      className={`relative overflow-hidden p-4 ${
        round ? 'rounded-full border border-white' : 'rounded-[24px] border border-white/10 bg-black/40'
      }`}
      style={{
        width: `${baseWidth}px`,
